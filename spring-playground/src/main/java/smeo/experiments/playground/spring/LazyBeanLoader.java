		...
		private Document makeDocumentDefaultLazyInitIfNotSet(Document document) {
			...
			for (int i = 0; i < beans.getLength(); i++) {
				...
				if (item.getNodeName().trim().equalsIgnoreCase("bean")) {
					final Element nodeAsElement = (Element) item;
					final String existingLazyInitValue = nodeAsElement.getAttribute(LAZY_INIT);
					if (existingLazyInitValue == null || existingLazyInitValue.isEmpty()) {
						((Element) item).setAttribute(LAZY_INIT, "true");
					} else {
						System.out.println("existing already!");
					}
				}
			}
			return document;
