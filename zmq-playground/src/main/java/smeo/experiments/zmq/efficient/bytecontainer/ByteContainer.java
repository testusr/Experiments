...
public class ByteContainer  {
    ...
    int writePos = 0;
    int readPos = 0;
    boolean byteArrayContainer = false;

    public static ByteContainer byteArrayContainer(long capacity){
        ByteContainer byteContainer = new ByteContainer(capacity);
        byteContainer.byteArrayContainer = true;
        return byteContainer;
    }

    public byte[] bytes(){
        if (byteArrayContainer){
            return data[0];
        }
        throw new IllegalArgumentException("not a byte array container");
    }

    private void grow(long noOfNeededBytes) {
        if (allocatedBlocks + 1 == data.length | byteArrayContainer) {
            throw new RuntimeException("max capacity reached");
        }
        long leftBytesAfterAdd = capacity - writePos - noOfNeededBytes;
    ...
    private void b(int writPos, byte value) {
        final int i = writPos / blockSize;
        final int j = writPos % blockSize;
        if (i >= allocatedBlocks) {
            grow();
        }
        // System.out.println("i: " + i + " j:" + j + " data.length: " + data.length + " allocatedBlocks: " + allocatedBlocks);
        data[i][j] = value;
    }
    ...
    public int read(byte[] b, int len) {
        int readBytes = read(b, readPos, len);
        readPos += len;
        return len;
    }
